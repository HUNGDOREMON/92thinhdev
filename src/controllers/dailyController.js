...
const giftPage = async (req, res) => {
  ...
  let money2 = 0;
  if (rows.length != 0) {
    const [point_list] = await connection.execute('SELECT `money`, `money_us` FROM `point_list` WHERE `phone` = ?', [
      rows[0].phone,
    ]);
...
const settings = async (req, res) => {
  ...
  if (!type) {
    const [point_list] = await connection.execute('SELECT `telegram` FROM `point_list` WHERE phone = ?', [
      rows[0].phone,
    ]);
...
const statistical = async (req, res) => {
  ...
  const [f1s] = await connection.query('SELECT `phone`, `code`,`invite`, `time` FROM users WHERE `invite` = ? ', [
    userInfo.code,
  ]);
  ...
  for (let i = 0; i < f1s.length; i++) {
    ...
    const [f2s] = await connection.query('SELECT `phone`, `code`,`invite`, `time` FROM users WHERE `invite` = ? ', [
      f1_code,
    ]);
    for (let i = 0; i < f2s.length; i++) {
      ...
      const [f3s] = await connection.query('SELECT `phone`, `code`,`invite`, `time` FROM users WHERE `invite` = ? ', [
        f2_code,
      ]);
      for (let i = 0; i < f3s.length; i++) {
        ...
        const [f4s] = await connection.query('SELECT `phone`, `code`,`invite`, `time` FROM users WHERE `invite` = ? ', [
          f3_code,
        ]);
  ...
  for (let i = 0; i < f1s.length; i++) {
    ...
    for (let i = 0; i < f2s.length; i++) {
      ...
      for (let i = 0; i < f3s.length; i++) {
        ...
        const [f4s] = await connection.query('SELECT `phone`, `code`,`invite` FROM users WHERE `invite` = ? ', [
          f3_code,
        ]);
...
const infoCtv = async (req, res) => {
  ...
  const [list_mem_baned] = await connection.query('SELECT * FROM users WHERE ctv = ? AND status = 2 AND veri = 1 ', [
    phone,
  ]);
  ...
  for (let i = 0; i < list_mem.length; i++) {
    let phone = list_mem[i].phone;
    const [wins] = await connection.query('SELECT `money`, `time` FROM minutes_1 WHERE phone = ? AND status = 1 ', [
      phone,
    ]);
    const [losses] = await connection.query('SELECT `money`, `time` FROM minutes_1 WHERE phone = ? AND status = 2 ', [
      phone,
    ]);
  ...
  let list_mems = [];
  const [list_mem_today] = await connection.query('SELECT * FROM users WHERE ctv = ? AND status = 1 AND veri = 1 ', [
    phone,
  ]);
...
const listRedenvelops = async (req, res) => {
  ...
  let userInfo = user[0];
  let [redenvelopes] = await connection.query('SELECT * FROM redenvelopes WHERE phone = ? ORDER BY id DESC', [
    userInfo.phone,
  ]);
  return res.status(200).json({
...
const listMember = async (req, res) => {
  ...
  const [total_users] = await connection.query(`SELECT * FROM users WHERE veri = 1 AND level = 0 AND ctv = ? `, [
    userInfo.phone,
  ]);
  return res.status(200).json({
    message: 'Success',
...
const listRechargeP = async (req, res) => {
  ...
  const [list_mem] = await connection.query('SELECT * FROM users WHERE ctv = ? AND status = 1 AND veri = 1 ', [
    userInfo.phone,
  ]);
  let list_recharge_news = [];
  for (let i = 0; i < list_mem.length; i++) {
