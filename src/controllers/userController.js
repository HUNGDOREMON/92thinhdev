...
import doRequest from "../bussiness/doRequest";
...
const getCoinApi = async (req, res) =>{
  ...
  let infoApi = await doRequest(params);
  if(!!infoApi && infoApi.status == 0){
    //console.log(infoApi)
    return res.status(200).json({
      message: "thanh congg",
      status: true,
      money_user: infoApi.balance,
      timeStamp: timeNow,
    });
  }else{
    return res.status(200).json({
...
const transferCoin = async(req, res)=>{
  let auth = req.cookies.auth;
  let type = req.body.type;// deposit , withdraw
  ...
  let reference_no = ''+randomNumber(1000000000, 9999999999);
  let UserInfo = rows[0];
  //(username, type, amount, reference_no)
  let RQ = await TC_API.transfer(UserInfo.phone, type, money2, reference_no);
  if(type == 'deposit'){
    //nap tien
    if(RQ.status == 0 && RQ.transaction_status == 'SUCCESS'){
    ...
    }else if(RQ.transaction_status == 'PENDING'){
      await connection.query( "UPDATE users SET money = money - ? WHERE `phone` = ? ", [money, UserInfo.phone]);
      return res.status(200).json({
        message: "Giao Dịch Đang Chờ Duyệt",
        status: true,
        timeStamp: timeNow,
      });
    }else {
...
const listGame = async (req, res) => {
  let auth = req.cookies.auth;
  let { type, game } = req.params;

  let operatorcode = process.env.operatorcode;
  let Secret_Key_Casino = process.env.Secret_Key_Casino;
  let passwordCasino = process.env.passwordCasino;
  ...
  // check user
  let signatureA = md5(
    String(operatorcode) + String(userInfo.phone) + String(Secret_Key_Casino)
  ).toUpperCase();
  await axios
    .get(
      `https://gsmd.336699bet.com/createMember.aspx?operatorcode=${operatorcode}&username=${userInfo.phone}&signature=${signatureA}`
    )
  ...
  await axios
    .get(
      `https://gsmd.336699bet.com/launchGames.aspx?operatorcode=${operatorcode}&providercode=${type}&username=${userInfo.phone}&password=${passwordCasino}&type=${game}&gameid=&lang=en-us&html5=0&signature=${signatureC}&blimit=xx`
    )
